...
from app_scheduler.configuracion import RAIZ_PROYECTO
...
def eliminar_cuarentena_factory_reset(operaciones):
    ...
    try:
        if factory_backups.is_dir() and not factory_backups.is_symlink() and not any(factory_backups.iterdir()):
            factory_backups.rmdir()
    except OSError:
        errores.append("factory_backups:OSError")
    return {"ok": not errores, "errores": errores}
...
def _resolver_root(clave, defecto):
    ...
    if not ruta_configurada.is_absolute():
        ruta_configurada = RAIZ_PROYECTO / ruta_configurada
    ...
    if ruta == RAIZ_PROYECTO or ruta.parent == ruta:
